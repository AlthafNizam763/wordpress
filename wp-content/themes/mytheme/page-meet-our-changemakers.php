...
<section class="page-header animate-on-scroll" style="background-color: #f4f4f4; padding: 60px 0; text-align: center;">
    <div class="container">
        <h1 style="font-size: 48px; margin-bottom: 20px; text-transform: uppercase; color:#000000ff">Meet Our
            Changemakers</h1>
    </div>
</section>
...
<main class="container" style="padding: 60px 20px;">
    <div class="gallery-masonry" style="column-count: 4; column-gap: 20px;">
        <?php
        $changemaker_path = get_template_directory() . '/images/changemaker/';
        $changemaker_url = get_template_directory_uri() . '/images/changemaker/';

        // Get all image files
        $images = glob($changemaker_path . '*.{jpg,jpeg,png,gif}', GLOB_BRACE);
        ...
        if ($images) {
            foreach ($images as $image) {
                ...
                echo '<a href="' . $changemaker_url . $image_name . '" target="_blank">';
                echo '<img src="' . $changemaker_url . $image_name . '" alt="Gallery Image" style="width: 100%; height: auto; display: block; transition: transform 0.5s ease;" onmouseover="this.style.transform=\'scale(1.05)\'" onmouseout="this.style.transform=\'scale(1)\'">';
                echo '</a>';
                echo '</div>';
            }
        } else {
